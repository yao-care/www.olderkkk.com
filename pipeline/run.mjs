...
- 特別注意「調整」二字：它是硬禁詞，任何日常用法都不行（「調整坐姿」「調整呼吸」「調整節奏」「微調整」全都會被退回）。改講：換個坐法、把坐姿換一下、把骨盆帶回中立、把呼吸放長、放慢節奏、重新學會發力。
...
【文風：去 AI 味】長短句交錯、每段換不同開法、每段一個具體事實、用台灣口語（容許口語瑕疵）。避免這些 AI 腔：破折號（兩條橫線連用的那個符號）是硬禁字元，全文任何位置都不准出現，連舉例、下定義、補充說明都不行，一律改用逗號或冒號。其他禁項：『不是X而是Y』『不僅…更』『並非…而是』的公式對比句、『值得注意的是』『值得一提的是』『換句話說』、空泛收束（總的來說／綜上所述／總而言之／歸根結底／整體而言）、『真正的問題/關鍵是』『隨著…的發展』『在…的今天』、『至關重要／不可或缺／舉足輕重』、模糊引用（研究顯示／專家認為／普遍認為，要嘛附來源要嘛不寫）、模板化第一人稱開場。要對比就正面直述。內部連結只用 Markdown（禁 raw HTML）。

【輸出】只輸出這篇 .md 的完整內容，從第一行的 --- 開始。不要任何前言、說明或 code fence。`;
}

// 閘門退回時的送修 prompt：最小幅度改寫移除違規，其餘一字不動。
// 禁詞清單直接抄合規閘門（guard/config），含 frontmatter（title/summary/faq）也算全文。
function buildRepairPrompt(md, reason) {
  return `以下這篇文章被自動合規閘門退回，理由：${reason}。

請整篇重新輸出同一篇文章，只做「最小幅度改寫」把所有違規之處移除；沒違規的句子一字不動。

【全部合規規則（含 frontmatter 的 title/summary/faq 都算全文，一併檢查）】
- 硬禁詞（子字串命中即退回）：治療、根治、療效、治好、痊癒、醫療行為、整骨、整脊、整復、整椎、正骨、無痛、脊椎、脊柱、調理、調整、端正、牽引。
  常見替代：脊椎/脊柱→背部、上背、腰背；調整→換個坐法、把骨盆帶回中立、放慢節奏；治療→舒緩、放鬆、保養。
  FAQ 問句也不可用禁詞（讀者問「是脊椎有問題嗎」要改寫成「是腰背出狀況嗎」這類講法）。
- 「矯正」只能寫成「運動矯正…」，不可落單。
- 禁破折號（兩條橫線連用的符號）與 AI 腔套語（值得注意的是、值得一提的是、綜上所述、總而言之、歸根結底、舉足輕重、研究顯示、隨著…的發展）。
- 必須保留：就醫/專業安全提醒、指向 [運動矯正・肌力訓練](/method) 的內部連結、完整 frontmatter（title/date/summary/order/faq）。

【輸出】只輸出修好後這篇 .md 的完整內容，從第一行的 --- 開始。不要任何前言、說明或 code fence。
...
async function main() {
  ...
  let md = normalizePunct(extractMd(raw));
  let v = lint(md);
  if (!v.ok) {
    // 閘門未過先送修一輪（最小改寫移除違規），還不過才 BLOCK——高頻通用禁詞（調整/脊椎）靠一次生成很難全避開
    err(`閘門未過（${v.reason}），送修一輪`);
    try {
      const fixed = normalizePunct(extractMd(await askText(buildRepairPrompt(md, v.reason))));
      md = fixed;
      v = lint(fixed);
    } catch (e) { err(`送修失敗：${(e.message || '').slice(0, 120)}`); }
  }
  if (!v.ok) {
    // 被擋的草稿落檔供稽核（.cache 在 .gitignore，不會進 repo）；否則犯規句無從查起
    try {
      await fs.mkdir('pipeline/.cache', { recursive: true });
      await fs.writeFile(`pipeline/.cache/blocked-${t.slug}-${Date.now()}.md`, md);
    } catch { /* 稽核落檔失敗不影響主流程 */ }
    status(`STATUS=BLOCK|${t.slug}|${v.reason}`);
    return;
  }
